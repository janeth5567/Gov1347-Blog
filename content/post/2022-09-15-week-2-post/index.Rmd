...
tags:
  ...
  - Prediction
  - Unemployment
excerpt: This weeks blog post will focus on using the economy as a variable to predictive modeling for the upcoming election using past data such as the unemployment rate, GDP for each quarter, RDI, etc.
---
...
```{r, echo  = FALSE}
fullmodel = lm(H_incumbent_party_majorvote_pct ~ 1+ GDP_growth_qt + DSPIC_change_qt + UNRATE , data = economy_model_data)
...
data_mod <- data.frame(Predicted = predict(fullmodel, new_data = National_prediction),  # Create data for ggplot2 
                       Observed = economy_model_data$H_incumbent_party_majorvote_pct)
